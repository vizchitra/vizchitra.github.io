...
function generateMarkdownReport(results, summary) {
	...
	const otherResults = results.filter(
		(r) => !r.file.includes('/components/') && !r.file.includes('/routes/') && r.hasIssues
	);
	...
	if (componentResults.length > 0) {
		...
		for (const [category, info] of Object.entries(ISSUE_CATEGORIES)) {
			const categoryComponentResults = componentResults.filter(
				(r) => r.issues[category].length > 0
			);
			...
			categoryRouteResults.forEach((result) => {
				const routeName =
					result.file
						.replace('src/routes/', '')
						.replace('/+page.svelte', '')
						.replace('/+layout.svelte', ' (layout)') || 'index';
				result.issues[category].forEach((issue) => {
	...
	results.forEach((result) => {
		Object.entries(result.issues).forEach(([category, issues]) => {
			issues.forEach((issue) => {
				const existing = topIssues.find(
					(t) => t.type === issue.type && t.details === issue.details
				);
				if (existing) {
