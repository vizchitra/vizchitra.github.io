<script lang="ts">
	...
	import { getDividerColorList, shuffleArray } from '$lib/utils/color';

	let width: number | null = $state(null);
	const height = 80;

	// Get concrete colors from the shared color util (normal variants)
	let resolvedColors: string[] = $state(
		getDividerColorList(['yellow', 'teal', 'blue', 'orange', 'pink'])
	);
...
<div class="divider-container my-xl w-full" bind:clientWidth={width}>
