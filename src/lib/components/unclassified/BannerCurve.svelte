<script lang="ts">
  ...
  const resolvedColorsList = getBannerColorList([
    "orange",
    "yellow",
    "pink",
    "teal",
  ...
  const getColor = (index: number) => {
    const shuffledIndex =
      shuffledColorOrder[index % shuffledColorOrder.length] ?? index;
  ...
  function generateCurvePoints(
    ...
    for (let i = 0; i < POINTS_PER_CURVE; i++) {
      ...
      const wave1 =
        Math.sin(xProgress * Math.PI * 2 + curveIndex * 0.5) * amplitude * 0.7;
      const wave2 =
        Math.sin(xProgress * Math.PI * 3 + curveIndex * 0.3) * amplitude * 0.4;
      const wave3 =
        Math.sin(xProgress * Math.PI * 5 + curveIndex * 0.2) * amplitude * 0.2;
