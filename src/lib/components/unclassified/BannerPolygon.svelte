<script lang="ts">
  ...
  import { getBannerColorList } from "$lib/utils/color.js";
  ...
  const _baseNames = ["yellow", "teal", "blue", "orange", "pink"];
  let resolvedColors: string[] = $state(getBannerColorList(_baseNames));
  ...
  const getColor = (index: number) =>
    resolvedColors[index % resolvedColors.length] || "#000000";
  ...
  function draw() {
    ...
    if (voronoi) {
      for (let i = 1; i < points.length; i++) {
        ...
        const centerY =
          cell.reduce((sum: number, p: number[]) => sum + p[1], 0) /
          cell.length;
        let alpha = 0.8;
