<script>
  ...
  let pentagonSize = $state(memberData?.talkType === "keynote" ? 425 : 275);
  let pentagonXOffset = $derived(memberData?.talkType === "keynote" ? 120 : 20); // Adjust as needed for positioning
  let pentagonYOffset = $state(memberData?.talkType === "keynote" ? 0 : 20);
  ...
  function computePoints(points, pentagonSize) {
    ...
    clipPath = adjustedPoints
      .map(
        (point) =>
          `${(point.x * 100) / pentagonSize}% ${(point.y * 100) / pentagonSize}%`,
      )
      .join(", ");
...
{#if colorMapping}
  <div
    class="flex h-full w-full flex-row items-start justify-between gap-sm md:gap-lg"
  >
    <div class="pentagon-container relative h-full w-full">
      <div
        ...
        style="transform: translate({-50}px, {points[1].y +
          pentagonYOffset +
          (isKeynote && screenWidth > 550
            ? 0
            : -55)}px); transform-origin: top right;"
      >
        {@html SpeakerCardPattern1.replaceAll(
          ...
          .replaceAll(
            "#F89F72",
            colorMapping[memberData.talkType]?.patternGradient[1],
          )
          .replaceAll(
      ...
      <div
        ...
        style="transform: translate({0}px, {isKeynote && screenWidth > 550
          ? 0
          : 75}px); "
      >
      ...
      >
        <path
          ...
          d="M {-(svgWidth - pentagonSize + pentagonXOffset)},0 
				...
				L {-(svgWidth - pentagonSize + pentagonXOffset)},{points[1].y +
            pentagonYOffset} Z"
        ...
        <g
          ...
          transform="translate({svgWidth -
            pentagonSize +
            pentagonXOffset},{pentagonYOffset})"
        >
      ...
      <div
        ...
        style="clip-path: polygon({clipPath}); --strength-color: {colorMapping[
          memberData.talkType
        ]
