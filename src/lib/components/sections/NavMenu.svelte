...
<nav class="sticky top-0 right-0 left-0 z-50 bg-white">
	<div class="px-md lg:px-lg mx-auto max-w-7xl">
		<div class="py-sm flex items-center justify-between">
			...
			<div class="mobile-drawer-container relative flex items-center justify-center">
				<!-- <a
			...
			<div class="desktop-drawer-container font-display ml-sm gap-xs flex items-center">
				{#each navSections as section}
							...
							{#if section.expanded}
								...
								>
									{#each section.subsections as subsection}
										...
										>
											<span
												class="text-grey-dark gap-xs flex items-center text-xl font-medium whitespace-nowrap"
											>
												{subsection.name}
												{#if subsection.isBadge}
													<span
														class="ml-xs px-xs.5 rounded-full border border-blue-500 bg-linear-to-r from-blue-400 to-blue-600 py-0.5 text-xs font-semibold text-white shadow-sm transition-transform duration-200 hover:scale-105"
													>
					...
					{:else}
						...
						>
							<span class="font-base text-viz-grey-dark text-xl whitespace-nowrap"
								>{section.name}</span
							>
						</a>
					{/if}
