...
{#if expanded}
	<div
		class="drawer font-display gap-xs p-sm absolute right-[-16px] flex max-h-[90dvh] flex-col overflow-auto rounded-lg border-[1px] border-[#ddd] bg-white shadow-lg sm:right-[-24px] lg:right-[-32px]"
		use:slideInDrawer
	>
			...
			<div class="nav-section">
				...
				<div class="links mb-1">
					{#if section?.subsections}
						...
						<div
							class="sub-section-list px-xs py-xs relative flex flex-col rounded-md bg-white"
							style:--accent-color={section.accentColor}
						>
							{#each section.subsections as subsection}
								<a
									href={subsection.href}
									class="subsection py-xs w-full cursor-pointer"
									onclick={handleClick}
								>
