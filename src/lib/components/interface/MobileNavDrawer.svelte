...
{#if expanded}
  <div
    class="drawer font-display absolute right-[-16px] flex max-h-[90dvh] flex-col gap-xs overflow-auto rounded-lg border-[1px] border-[#ddd] bg-white p-sm shadow-lg sm:right-[-24px] lg:right-[-32px]"
    use:slideInDrawer
  >
    {#each navSections as section, index}
      ...
      <div class="nav-section">
          ...
          <div class="polygon-divider mb-2 w-full" bind:clientWidth={width}>
            ...
            >
              ...
              <line
                ...
                y2={height / 2}
                stroke={colors[
                  Math.floor(Math.random() * colors.length + 2) % colors.length
                ]}
                stroke-width={8}
              ></line>

              <circle cx="{pointStartingX}%" cy="50%" r={5 + 4} fill={"white"}
              ></circle>
              <circle cx="{pointStartingX}%" cy="50%" r={5} fill={"#4c4c4c"}
              ></circle>
            </svg>
          </div>
        {/if}
        ...
        <div class="links mb-1">
          {#if section?.subsections}
            <span class="w-full text-left">
              <span class="font-base text-2xl whitespace-nowrap text-[#4C4C4C]"
                >{section.name}</span
              >
            </span>
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div
              class="sub-section-list relative flex flex-col rounded-md bg-white px-xs py-xs"
              style:--accent-color={section.accentColor}
            >
              {#each section.subsections as subsection}
                <a
                  href={subsection.href}
                  class="subsection w-full cursor-pointer py-xs"
                  onclick={handleClick}
                >
                  <span
                    class="font-base text-xl whitespace-nowrap text-[#4C4C4C]"
                    >{subsection.name}
        ...
        <div class="polygon-divider mb-xs w-full" bind:clientWidth={width}>
          <svg
            {width}
            {height}
            viewBox="0 0 {width ?? 0} {height ?? 0}"
