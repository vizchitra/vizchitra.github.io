...
{#if state.loading}
	<div class="{height} w-full {className} flex items-center justify-center bg-gray-100 rounded">
		<div class="text-gray-500">Loading map...</div>
	</div>
{:else if state.error}
	<div class="{height} w-full {className} flex items-center justify-center bg-red-50 rounded border border-red-200">
		<div class="text-red-600 text-sm">{state.error}</div>
	</div>
