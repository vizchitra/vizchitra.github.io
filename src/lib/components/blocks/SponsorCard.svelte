<!--
	SponsorCard - 3D pentagon card for sponsor display.
	Features seeded randomization for unique card variations.
-->
<script lang="ts">
	interface Props {
		/** Card heading text */
		heading?: string;
		/** Logo content (text, image URL, or HTML) */
		logo?: string;
		/** How to render the logo */
		logoType?: 'text' | 'image' | 'html';
		/** Color variant */
		variant?: 'pink' | 'yellow' | 'orange' | 'blue' | 'green' | 'white' | 'black';
		/** Size variant */
		size?: 'small' | 'medium' | 'large';
		/** Seed for shape variations (ensures unique but consistent cards) */
		seed?: number;
	}
	...
	// Color variants
	const variants = {
		pink: { light: '#f8d7da', dark: '#e91e63' },
		yellow: { light: '#fff8dc', dark: '#ffa500' },
		orange: { light: '#F89F72', dark: '#661e00' },
		blue: { light: '#e3f2fd', dark: '#2196f3' },
		green: { light: '#e8f5e8', dark: '#4caf50' },
		white: { light: '#F6F6F6', dark: '#4C4C4C' },
		black: { light: '#4C4C4C', dark: '#3B3B3B' }
	} as const;

	// Size variants
	const sizes = {
		small: { width: '250px', height: '180px', fontSize: '14px', logoSize: '18px' },
		medium: { width: '350px', height: '250px', fontSize: '16px', logoSize: '24px' },
		large: { width: '450px', height: '320px', fontSize: '20px', logoSize: '32px' }
	} as const;
	...
	function getShapeVariations(seed: number) {
		...
		const variationRange = 8;
		const shadowVariationRange = 3;
	...
	const currentVariant = $derived(variants[variant] ?? variants.pink);
	const currentSize = $derived(sizes[size] ?? sizes.medium);
	const shapeVariations = $derived(getShapeVariations(seed));
</script>
