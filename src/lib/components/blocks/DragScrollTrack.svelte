...
{#if showButtons && (atStart === false || atEnd === false)}
	...
	<div class="track-controls pl-sm sm:gap-sm flex items-center gap-sm self-start">
		<button
			onclick={() => scrollTrack('left')}
			disabled={atStart}
			class="size-xl border-grey bg-pink-light sm:size-2xl flex cursor-pointer items-center justify-center rounded-full border p-sm hover:opacity-70 disabled:bg-white disabled:opacity-60"
