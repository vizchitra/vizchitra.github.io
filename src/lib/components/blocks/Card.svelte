...
{#if isBordered}
	{#if href}
		<a {href} class="group inline-block w-full no-underline">
			<Stack space="sm" class="p-sm rounded-lg border-2 {tokens.border} group-hover:shadow-md transition-shadow group-hover:bg-opacity-50">
				{#if title}
...
{:else}
	{#if href}
		<a {href} class="group block no-underline">
			<Stack space="md" direction="row" class="items-start group-hover:shadow-md transition-shadow">
				{#if image}
	...
	{:else}
		<Stack space="md" direction="row" class="items-start">
			{#if image}
				<div class="w-full shrink-0 md:w-1/3">
					<img src={image} {alt} class="h-auto w-full rounded-sm object-cover" />
				</div>
