<script lang="ts">
	...
	import { ProposalBadge, ProposalStatusBadge } from '$lib/components/proposals';
...
<div class="content-container max-w-4xl">
	<div class="border-viz-grey/10 mb-6 border-b pb-4 md:mb-8 md:pb-6">
		<div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between md:gap-6">
			<div class="order-2 flex flex-wrap items-center gap-1.5 text-sm md:order-1 md:gap-2">
				...
				>
					{#if proposal.materials}
						<div>
							...
							<p class="text-viz-grey/90 text-sm break-words md:text-base">{proposal.materials}</p>
						</div>
					{/if}
					{#if proposal.roomSetup}
						<div>
							...
							<p class="text-viz-grey/90 text-sm break-words md:text-base">{proposal.roomSetup}</p>
						</div>
					{/if}
				</div>
			</section>
	...
	{#if isCFE}
		...
		{#if proposal.submissionType === 'Collective' && proposal.teamName}
			<section class="mb-8 md:mb-10">
				...
				<div class="border-viz-orange-dark bg-viz-orange-light/10 rounded-lg border-l-4 p-4 md:p-6">
